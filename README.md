...
1. 채팅방 다른 채팅방에서 사용했는데 일반 채팅(메인, 첫번째)에서 채팅이 출력된다
2. 몇몇 명령어들이 뭔가 부실하다
3. 플레이리스트링크를 넣으면 노래가 모두 넣어지도록 하는 것을 노리고 있는데 잘 안된다. 다른 노래 봇들과 비교해서 현저히 느린 로딩속도
4. /반복을 사용할때 토글로 명령어가 사용되도록 되있는데 대기열반복 상태일때 반복이 안되는 오류가 있다.
제일 유력한 문제가 있다고 생각하는 코드부분이 대기열 부분이라 이 부분 유의해서 보면 해결되는 문제가 많을듯하다
